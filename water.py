...
def initial_state():
    # We start from the state where 19 gallons is empty and other are full
    return State(0, 13 , 7)

# Define how to check if a state is a goal state
def is_goal(s):
    # It is a goal when one of bottle is 10 gallons
...
def successors(s):
    ...
    # Case 3: Try to empty the bottle Y
    if s.z > 0:
        yield (State(s.x, s.y, 0), s.z)

    # Case 4: Try to pour water from X to Y
    t = 13-s.y  
    if s.x > 0 and t > 0:
        if s.x > t:
    ...
    # Case 5: Try to pour water from X to Z
    t = 7-s.z   
    if s.x > 0 and t > 0:
        if s.x > t:
            # Pour until Y is full
    ...
    if s.y > 0 and t > 0:
        if s.y > t:
            ...
            yield (State(3, s.y-t, s.z), t)
        else:
            # Pour until Y is empty
            yield (State(s.x+s.y, 0 , s.z), s.y)

    # Case 7: Try to pour water from Y to Z
    t = 7-s.z   
    if s.y > 0 and t > 0:
        if s.y > t:
